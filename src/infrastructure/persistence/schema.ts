 ...
 *
 * Slice 6 adds `projection_field_provenance`: one row per (entity, field),
 * naming the event that last wrote it and carrying that event's provenance
 * columns flattened, exactly as `events` holds them.
 *
 ...
 *
 * The provenance columns are duplicated from `events` rather than joined to on
 * `event_id`, which is denormalisation and is the point: §7's argument is that
 * building the pointer during projection is cheap and reconstructing it later
 * is not. A join to the log on every field would be reconstruction with extra
 * steps, and the log is the one table the read path is not supposed to touch
 * (`add.md` §6).
 *
 * ## `projection_redirects` is declared here and written in Slice 8
 *
 ...
 * projection machinery is the honest place for it, and because a `projection_`
 * table added later is a rebuild rather than a migration either way.
 *
 ...
 * it the one projection that cannot be rebuilt by replaying the log, which is
 * the property the `projection_` prefix is a promise about.
 */
