...
- **Build and packaging pipeline.** How the sidecar, `whisper.cpp`, the embedding model, and the vector extension are bundled into a Tauri installer per platform. Slice 1 introduces `src-tauri/` and pins the Rust toolchain, the Tauri major version, and the audio-capture crate; the installer, signing, and per-platform bundling stay open until Slice 10.
