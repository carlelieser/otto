...
**The write path is a chain, not a set.** Extraction cannot be built before Captures exist, resolution cannot be built before entities exist, and triage cannot be built before proposals exist. Slices 1–5 follow that chain in order because no reordering of them is available.
...
| 1 | [Capture](./01-capture.md) | Typed and voice capture to a durable, immutable Capture | 0 |
| 2 | [Extraction](./02-extraction.md) | A Capture becomes Mentions and field values | 1 |
| 3 | [Resolution and the differ](./03-resolution-and-differ.md) | Mentions become Commands against real entities | 2 |
| 4 | [Triage](./04-triage.md) | Commands become applied events or queued Proposals | 3 |
| 5 | [Projections and the read path](./05-projections-and-read-path.md) | Knowledge is browsable with provenance | 4 |
| 6 | [Review queue](./06-review-queue.md) | The user confirms, corrects, and sees what Otto did | 5 |
| 7 | [Duplicates and merge](./07-duplicates-and-merge.md) | Two entities that are one become one | 6 |
| 8 | [Transcript correction](./08-transcript-correction.md) | A misheard name is fixable in one step | 6 |
| 9 | [Salience and briefs](./09-salience-and-briefs.md) | What deserves attention is surfaced daily and weekly | 5 |
| 10 | [Dashboard and packaging](./10-dashboard-and-packaging.md) | Otto is an application the user installs and lives in | 5, 6 |

```mermaid
flowchart LR
    S0["0 · Foundation"] --> S1["1 · Capture"] --> S2["2 · Extraction"] --> S3["3 · Resolution<br/>+ differ"] --> S4["4 · Triage"] --> S5["5 · Projections<br/>+ read path"]
    S5 --> S6["6 · Review queue"]
    S5 --> S9["9 · Salience<br/>+ briefs"]
    S6 --> S7["7 · Duplicates<br/>+ merge"]
    S6 --> S8["8 · Transcript<br/>correction"]
    S5 --> S10["10 · Dashboard<br/>+ packaging"]
    S6 --> S10
```

Slices 0–6 are a chain: each needs the one before it. After 6, three branches are independent of one another — 7, 8, and 9 touch different surfaces and can be built in any order or in parallel. Slice 10 is last because it is the one that has to see all of them.

## What is deliberately not a slice

**The local-extraction measurement is a gate, not a slice.** ADR-0013 and `runtime.md` §2 name it as the assumption in Otto most likely to be wrong, and PRD §9 lists it as gating implementation. It runs inside Slice 2 as that slice's exit condition, because the thing being measured is exactly what Slice 2 builds. If it fails, the response is a larger minimum local model, never looser thresholds.

**The SQLite spike is done.** It passed on all seven bars (`runtime.md` §4) and its measurements become the standing performance suite, which Slice 5 stands up against the real projector.

**Split is post-MVP** (PRD §7.2, ADR-0009). Slice 7 ships merge and tests that no split path exists.
