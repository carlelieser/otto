...
> **Outcome (spike run).** All seven bars passed, the closest by a factor of 20. Vector search — the predicted failure — cleared its bar by 330× via `sqlite-vec`, so the separate-index fallback is not needed. The decision below stands unchanged; results are in `runtime.md` §4 and `spikes/sqlite/FINDINGS.md`. The gate this ADR set is therefore half cleared: SQLite passed, and the local-extraction measurement has not yet been run.
